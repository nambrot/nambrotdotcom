...
(function() {
  var $, TimelineEntry, TimelineEntryManager,
    indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };

  $ = jQuery;

  TimelineEntry = (function() {
    function TimelineEntry() {}

    TimelineEntry.prototype.cached_top_position = 0;

    TimelineEntry.prototype.initialize_from_DOM_element = function(element, settings) {
      this.element = element;
      this.$element = $(element);
      this.settings = settings;
      this.categories = $.map(this.$element.find(this.settings.timeline_entry_tag_selector).text().replace(/^\s+|\s+$/g, '').split(' '), function(hashtag) {
        return hashtag.slice(1, hashtag.length);
      });
      this.importance = parseInt(this.$element.data('importance'));
      this.hidden = this.$element.hasClass('hidden');
      this.$element.find(this.settings.timeline_entry_card_selector).css({
        position: 'absolute'
      });
      return this;
    };

    TimelineEntry.prototype.card_height = function() {
      return this.$card_height != null ? this.$card_height : this.$card_height = this.$element.find(this.settings.timeline_entry_card_selector).height();
    };

    TimelineEntry.prototype.click = function(evt) {
      var detail_view, horizontaloffset, offset, scrolltop, transform, verticaloffset;
      if ($(window).width() < 500) {
        detail_view = $('<div class="reveal-modal" data-reveal><a class="close-reveal-modal">&#215;</a></div>').append(this.$element.find('.about-timeline-entry-detail').children().clone());
        detail_view.find('.close-reveal-modal').click(function() {
          return detail_view.foundation('reveal', 'close');
        });
        detail_view.appendTo('body').foundation('reveal', 'open');
        return;
      }
      this.overlay = $('<div id="container-overlay"></div>');
      $('body').append(this.overlay);
      this.overlay.fadeIn(300);
      scrolltop = $(window).scrollTop();
      offset = this.$element.offset();
      if ($(window).height() > this.$element.find('.about-timeline-entry-detail').height() / 2) {
        verticaloffset = $(window).height() / 2 + scrolltop - offset.top - this.$element.find('.about-timeline-entry-detail').height() / 4;
      } else {
      ...
      this.$element.find('.about-timeline-entry-detail').css({
        display: 'block'
      });
      this.$element.css({
      ...
      return setTimeout(((function(_this) {
        return function() {
          $(document).on('click.timelinify', function(evt) {
            if (!$.contains(_this.element, evt.target)) {
              return _this.flipback();
            }
          });
          $(document).on('keyup.timelinify', function(evt) {
            if (evt.keyCode === 27) {
              return _this.flipback();
            }
          });
          return _this.$element.find('.about-timeline-entry-detail').on('click.timelinify', function(evt) {
    ...
    TimelineEntry.prototype.flipback = function() {
      this.overlay.fadeOut(300, (function(_this) {
        return function() {
      ...
      this.$element.find('.about-timeline-entry-detail').off('click.timelinify');
      ...
      return setTimeout(((function(_this) {
        return function() {
          return _this.$element.find('.about-timeline-entry-detail').css({
            display: ''
          });
        };
      })(this)), 300);
    ...
    TimelineEntry.prototype.hide = function() {
    ...
    TimelineEntry.prototype.show = function() {
      this.$element.css({
        'position': 'absolute',
        'opacity': '1',
        'z-index': '',
      ...
      return setTimeout(((function(_this) {
        return function() {
          return _this.$element.find("img[data-gallery-decorate=true]").decorate_gallery();
        };
      })(this)), 1000);
    };
  ...
  TimelineEntryManager = (function() {
    function TimelineEntryManager() {}

    TimelineEntryManager.prototype.column_mapping = [];

    TimelineEntryManager.prototype.entries_currently_displayed = [];

    TimelineEntryManager.prototype.set_location_hash = function() {
      var acceptable_categories, importance_threshold;
      acceptable_categories = $(this.settings.category_picker_form + ' input:checked').map(function(index, value) {
        return value.value;
      });
      importance_threshold = parseInt($(this.settings.importance_slider).val());
      return window.location.hash = "categories:" + (acceptable_categories.toArray().join(',')) + ";importance:" + importance_threshold;
    };

    TimelineEntryManager.prototype.set_filters_from_location_hash = function() {
      var checked_categories, filter_name, filter_value, filters, input, ref, results;
      filters = window.location.hash.split(';');
      if (filters[0] === "") {
        return;
      }
      results = [];
      while (filters.length > 0 && (ref = filters.shift().split(':'), filter_name = ref[0], filter_value = ref[1], ref)) {
        switch (filter_name) {
          case "importance":
            console.log(this.settings);
            results.push($(this.settings.importance_slider).simpleSlider('setValue', filter_value));
            break;
          case "categories":
            checked_categories = filter_value.split(',');
            results.push((function() {
              var j, len, ref, results1;
              ref = $(this.settings.category_picker_form + ' input');
              results1 = [];
              for (j = 0, len = ref.length; j < len; j++) {
                input = ref[j];
                if (checked_categories.indexOf(input.value) === -1) {
                  results1.push(input.checked = false);
                } else {
                  results1.push(input.checked = true);
                }
              }
              return results1;
            }).call(this));
    ...
    TimelineEntryManager.prototype.initialize_from_DOM_elements = function(entries, settings) {
      if (settings == null) {
        settings = {};
      }
      this.entries_dom_elements = entries;
      this.timeline_entries = $.map(entries, function(val) {
        return (new TimelineEntry).initialize_from_DOM_element(val, settings);
      });
      this.settings = settings;
      $(this.settings.timeline_selector).on('click', this.settings.timeline_entry_selector + ".flippable", null, (function(_this) {
        return function(evt) {
          console.log('click timeline entry');
          return _this.timeline_entries[_this.entries_dom_elements.index(evt.currentTarget)].click(evt);
        };
      })(this));
      $(this.settings.category_picker_form + ' input').on('change', (function(_this) {
        return function() {
          _this.set_location_hash();
          return _this.reflow();
        };
      })(this));
      $(this.settings.importance_slider).simpleSlider();
      $(this.settings.importance_slider).on('change slider:changed', (function(_this) {
        return function() {
          _this.set_location_hash();
          return _this.reflow();
        };
      })(this));
      this.set_filters_from_location_hash();
      $(window).on("hashchange", (function(_this) {
        return function() {
          _this.set_filters_from_location_hash();
          return _this.reflow();
        };
      })(this));
      return this;
    ...
    TimelineEntryManager.prototype.reflow = function() {
      var acceptable_categories, column, entries_to_show, entry, get_column_height, height, i, importance_threshold, index, intersection, j, k, l, left, len, len1, len2, len3, m, maximum_column, maximum_height, minimal_column, minimum_height, number_of_columns, ref, ref1, ref2, top;
      ...
      get_column_height = function(column) {
        var last;
        if (column.length > 0) {
          last = column[column.length - 1];
          return last.cached_top_position + last.card_height();
        } else {
          return 0;
        }
      };
      ...
      importance_threshold = parseInt($(this.settings.importance_slider).val());
      entries_to_show = [];
      ref = this.timeline_entries;
      for (j = 0, len = ref.length; j < len; j++) {
        entry = ref[j];
        if (intersection(entry.categories, acceptable_categories).length > 0 && entry.importance <= importance_threshold) {
          entry.show();
      ...
      number_of_columns = parseInt($(this.settings.timeline_selector).width() / (this.settings.columnWidth + 2 * this.settings.columnMargin));
      if (number_of_columns === this.column_mapping.length && intersection(entries_to_show, this.entries_currently_displayed).length === entries_to_show) {
        return;
      }
      this.entries_currently_displayed = entries_to_show;
      this.column_mapping = (function() {
        var k, ref1, results;
        results = [];
        for (i = k = 0, ref1 = number_of_columns - 1; 0 <= ref1 ? k <= ref1 : k >= ref1; i = 0 <= ref1 ? ++k : --k) {
          results.push([]);
        }
        return results;
      })();
      for (k = 0, len1 = entries_to_show.length; k < len1; k++) {
        entry = entries_to_show[k];
        minimum_height = get_column_height(this.column_mapping[0]);
        minimal_column = 0;
        ref1 = this.column_mapping;
        for (index = l = 0, len2 = ref1.length; l < len2; index = ++l) {
          column = ref1[index];
        ...
        this.column_mapping[minimal_column].push(entry);
        if (minimum_height === 0) {
          top = 0;
        } else {
          top = minimum_height + this.settings.columnMargin;
        }
        if (number_of_columns !== 1) {
          left = minimal_column * (this.settings.columnWidth + this.settings.columnMargin);
        } else {
          left = ($(this.settings.timeline_selector).width() - this.settings.columnWidth - 2 * this.settings.columnMargin) / 2;
        }
        entry.$element.css({
          top: top,
          left: left
      ...
      maximum_height = get_column_height(this.column_mapping[0]);
      maximum_column = 0;
      ref2 = this.column_mapping;
      for (index = m = 0, len3 = ref2.length; m < len3; index = ++m) {
        column = ref2[index];
        height = get_column_height(column);
      ...
      return $(this.settings.timeline_selector).css('height', maximum_height);
    };

    return TimelineEntryManager;

  })();

  $.fn.extend({
    timelinify: function(options) {
      var log, settings;
      settings = {
      ...
      return this.each(function() {
        var entries_dom_elements, timeline_entry_manager;
        ...
        return $(window).resize(function() {
          return timeline_entry_manager.reflow();
        });
      });
    }
  });
...
(function() {
  $.fn.extend({
    gallery: function(options) {
      ...
      return this.each(function() {
        $.extend(this, {
          ...
          init: function() {
            this.loadNextPhotos();
            this.gallery.append('<span class="gallery-arrow-left"></span>');
            this.gallery.append('<span class="gallery-arrow-right"></span>');
            this.gallery.on('click', '.gallery-arrow-left', $.proxy(this.previous, this));
            this.gallery.on('click', '.gallery-arrow-right', $.proxy(this.next, this));
            this.gallery.append('<span class="gallery-fullscreen"></span>');
            this.gallery.on('click', '.gallery-fullscreen', $.proxy(this.requestFullScreen, this));
            $(document).on('webkitfullscreenchange mozfullscreenchange fullscreenchange', (function(_this) {
              return function() {
                _this.gallery.toggleClass('fullscreen');
                if (!_this.gallery.hasClass('fullscreen')) {
                  _this.gallery.height(_this.pre_fullscreen_height);
                  _this.canvas.find('img').width(300);
                  return _this.canvas.find('img').width('auto');
                }
              };
            })(this));
            return this.canvas.scroll((function(_this) {
              return function() {
                var child, i, index, len, ref, scrollLeft;
                scrollLeft = _this.canvas.scrollLeft();
                if (scrollLeft === 0) {
                  _this.currentPhoto = 0;
                } else {
                  ref = _this.canvas.children();
                  for (index = i = 0, len = ref.length; i < len; index = ++i) {
                    child = ref[index];
                    if (child.offsetLeft > scrollLeft) {
                      _this.currentPhoto = index - 1;
                      break;
                    }
                  }
                }
                if ((_this.canvas[0].offsetWidth + scrollLeft) * 100 / _this.canvas[0].scrollWidth > 60) {
                  return _this.loadNextPhotos(2);
                }
              };
            })(this));
          },
          loadNextPhotos: function(count) {
            var fullscreenEnabled, i, image, images, len, opts, results;
            if (count == null) {
              count = 4;
            }
            if (this.loading || this.gallery.find('img[loaded=false]').length === 0) {
              return;
            }
            this.loading = true;
            this.spinner_container = $('<div class="gallery-spinner"></div>');
            this.spinner_container.width((this.gallery.find('img[loaded=true]').length === 0 ? '100%' : '5%'));
            this.gallery.append(this.spinner_container);
            opts = {
              lines: 10,
              length: 5,
              width: 3,
            ...
            this.spinner = new Spinner(opts).spin(this.spinner_container[0]);
            images = this.gallery.find('img[loaded=false]').slice(0, count);
            $(images).imagesLoaded((function(_this) {
              return function() {
                return setTimeout((function() {
                  $(images).attr('loaded', true).addClass('loaded');
                  _this.loading = false;
                  _this.spinner.stop();
                  return _this.spinner_container.remove();
                }), 20);
              };
            })(this));
            fullscreenEnabled = this.gallery.hasClass('fullscreen');
            results = [];
            for (i = 0, len = images.length; i < len; i++) {
              image = images[i];
              if (fullscreenEnabled) {
                results.push(image.src = $(image).data('big-src'));
              } else {
                results.push(image.src = $(image).data('src'));
              }
            }
            return results;
          },
          next: function(evt) {
            var child, i, index, len, ref, results, scrollLeft;
          ...
          requestFullScreen: function() {
            var fullscreenEnabled, i, image, images, len;
            fullscreenEnabled = this.gallery.hasClass('fullscreen');
            if (fullscreenEnabled) {
              if (document.cancelFullScreen) {
                ...
                this.cancelFallbackFullscreen();
              }
              this.gallery.height(this.pre_fullscreen_height);
              this.canvas.find('img').width(300);
              this.canvas.find('img').width('auto');
              $('body').off("keydown.gallery_fullscreen");
            } else {
              this.pre_fullscreen_height = this.gallery.height();
              if (this.gallery[0].requestFullScreen) {
                this.gallery[0].requestFullScreen();
              } else if (this.gallery[0].mozRequestFullScreen) {
                this.gallery[0].mozRequestFullScreen();
              } else if (this.gallery[0].webkitRequestFullScreen) {
                this.gallery[0].webkitRequestFullScreen();
              } else {
                this.requestFallbackFullscreen();
              }
              this.gallery.height("100%");
              $('body').on("keydown.gallery_fullscreen", (function(_this) {
                return function(e) {
                  switch (e.which) {
                    case 37:
                      _this.previous(e);
              ...
              images = this.canvas.find('img.loaded');
              for (i = 0, len = images.length; i < len; i++) {
                image = images[i];
                if (image.src !== $(image).data('big-src')) {
                  (function(image) {
                    ...
                    $(loading_image).load((function(_this) {
                      return function() {
                        return image.src = loading_image.src;
                      };
                    })(this));
                    return loading_image.src = $(image).data('big-src');
                  })(image);
                }
              }
            }
            return setTimeout(((function(_this) {
              return function() {
                return _this.goToImage(_this.currentPhoto);
              };
            })(this)), 400);
          },
          requestFallbackFullscreen: function() {
            var fullscreen;
            this.parent = this.gallery.parent();
            fullscreen = $('<div id="gallery-fallback-fullscreen"></div>');
            $('body').append(fullscreen);
            this.gallery.detach();
            fullscreen.append(this.gallery);
            this.canvas = this.gallery.find('.gallery-album-photos-canvas');
            return this.gallery.addClass('fullscreen');
          },
          cancelFallbackFullscreen: function() {
            this.gallery.detach();
            $('#gallery-fallback-fullscreen').remove();
            this.parent.append(this.gallery);
            this.canvas.find('.gallery-album-photos-canvas');
            return this.gallery.removeClass('fullscreen');
          },
          goToImage: function(index) {
            this.currentPhoto = index;
            return this.canvas.animate({
              scrollLeft: this.canvas.children()[index].offsetLeft
            }, 300);
          }
        });
        this.init();
    ...
    decorate_gallery: function(options) {
      var settings;
      settings = {
        foo: 'bar'
      };
      ...
      return this.each(function() {
        var height;
        height = $(this).height();
        return $.getJSON("/gallery/albums/" + ($(this).data('gallery-id')) + "/photos", (function(_this) {
          return function(resp) {
            var canvas, i, image, len, replacement;
            if (resp.length === 0) {
              return;
            }
...
(function() {
  $(document).on('ready page:load', function() {
    ...
    return $('#about-page').addClass('timelinified');
  });
...
(function() {
  ...
  $.offCanvasMenu = function(options) {
    ...
    outerWrapper = $({});
    innerWrapper = $({});
    baseCSS = "<style>" + outer + " { left: 0; overflow-x: hidden; position: absolute; top: 0; width: 100%; }" + inner + " { position: relative; -webkit-backface-visibility: hidden; }" + "; } </style>";
    head.append(baseCSS);
    actions = {
      on: function() {
...
(function() {
  $(document).on('ready page:load', function() {
    ...
    return $('.gallery-album-decorate').decorate_gallery();
  });
